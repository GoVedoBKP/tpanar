...
#include "cmd_track_insert_silence.h"
#include "../mixer/track.h"
#include "../instrument/sample_instrument.h"

namespace tpanar_ns {

TrackInsertSilenceCommand::TrackInsertSilenceCommand(Track& track, size_t insert_sample, size_t duration_samples)
    : m_track(track), m_insert_sample(insert_sample), m_duration_samples(duration_samples)
{
}

void TrackInsertSilenceCommand::apply() {
    ...
    if (insert_pos > sample.data->left.size()) {
        insert_pos = sample.data->left.size();
    }

    std::vector<float> new_left;
    new_left.insert(new_left.end(), sample.data->left.begin(), sample.data->left.begin() + insert_pos);
    new_left.insert(new_left.end(), m_duration_samples, 0.0f);
    ...
    if (is_stereo) {
        ...
        new_right.insert(new_right.end(), m_duration_samples, 0.0f);
...
void TrackInsertSilenceCommand::undo() {
    auto inst = m_track.instrument();
    if (!inst || inst->type() != InstrumentType::Sampler) {
        return;
    }
    ...
    size_t end_pos = m_insert_sample + m_duration_samples;
